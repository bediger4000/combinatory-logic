...
int
graph_graph(struct node *node, FILE *fout, int reduction_node_sn, int counter)
{
	int r = counter, this_node_number = counter;

	if (node)
	{
		char* marker = (node->sn == reduction_node_sn)? "*": "";
		fprintf(fout, "n%x [label=\"%d%s\\n%s\"];\n", this_node_number, node->sn, marker, node->name? node->name: "@");
		if (APPLICATION == node->typ)
		{
			int child_counter;
			child_counter = graph_graph(node->left, fout, reduction_node_sn, counter + 1);
			if (node->left) fprintf(fout, "n%x -- n%x;\n", this_node_number, this_node_number + 1);
			r = graph_graph(node->right, fout, reduction_node_sn, child_counter + 1);
...
/* Call this with the "real" root, not the dummy created
 * before the call to reduce_graph(), which has a dummy
 * right child, too. */
void
graph_to_file(const char *filename, struct node *real_root)
{
	FILE *fout;

	if (NULL == (fout = fopen(filename, "w")))
	{
		fprintf(stderr, "Problem opening \"%s\" to write a graph in: %s\n",
			filename, strerror(errno));
		return;
	}

	fprintf(fout, "graph G%p {\n", real_root);
	graph_graph(real_root, fout, 0, 0);
	fprintf(fout, "}\n");
